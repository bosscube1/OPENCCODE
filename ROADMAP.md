...
| 2.5 | Slice coverage for the <20% offenders. **Partly done.** `uiSlice` 3.44% → **100%** (2026-08-05); `sessionSlice` and `eventSlice` were covered in earlier waves (now 88.27% / high). Remaining, measured 2026-08-05: `routingSlice` **2.08%** (0% branch), `projectsSlice` **4%** (0% branch), `compareSlice` **4.08%**, `imagesSlice` **10.34%**, `api.ts` **30.76%**, `gitSlice` **59.32%**. Take them worst-first — each is the same shape of work as `uiSlice.test.ts`. | `lib/__tests__/` |
