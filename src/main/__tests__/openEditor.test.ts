...
vi.mock('../server', () => ({
  getEventDirectory: () => state.directory
}))
...
describe('openEditor', () => {
  ...
  it('opens a vscode:// deep link for a contained relative path', async () => {
    await openEditor({ path: 'src/foo.ts' })
  ...
  it('appends :line and :line:column when provided', async () => {
    await openEditor({ path: 'src/foo.ts', line: 12 })
    expect(state.openExternal.mock.calls[0][0]).toMatch(/:12$/)

    await openEditor({ path: 'src/foo.ts', line: 12, column: 3 })
  ...
  it('rejects a path that escapes the project directory', async () => {
    await expect(openEditor({ path: '../outside.ts' })).rejects.toThrow(/strictly inside/i)
  ...
  it('rejects an absolute path outside the project directory', async () => {
    ...
    try {
      await expect(openEditor({ path: join(outside, 'evil.ts') })).rejects.toThrow(/strictly inside/i)
    } finally {
  ...
  it('throws when there is no active project directory', async () => {
    state.directory = null
    await expect(openEditor({ path: 'src/foo.ts' })).rejects.toThrow(/no active project directory/i)
    expect(state.openExternal).not.toHaveBeenCalled()
  })
  ...
  describe('register', () => {
    it('wires the oc:openEditor IPC channel and validates arguments', async () => {
      ...
      await handler({}, { path: 'src/foo.ts', line: 4 })
      expect(state.openExternal).toHaveBeenCalledTimes(1)

      await expect(handler({}, { path: 123 })).rejects.toThrow(/path must be a non-empty string/i)
      await expect(handler({}, { path: 'src/foo.ts', line: -1 })).rejects.toThrow(/line must be a positive integer/i)
